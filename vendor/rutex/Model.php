...
class Model
{
    ...
    function RequiredFields_Verify($data)
    {
        ...
        foreach ($this->struct as $fieldName => $def) {
            if (isset($data[$fieldName]) && $data[$fieldName])
                $this->record[$fieldName] = $data[$fieldName];
        ...
        if ($verified)
            $this->setResult(true, "Todos los campos obligatorios están cargados");
        else
            $this->setResult(false, "Faltan datos obligatorios: " . implode(", ", $requiredFields));
    ...
    private function makeSelectCmd($fields = "*", $page = 0, $rows = 0)
    {
        ...
        if (empty($whereCondition))
            $sqlcmd = "select {$fields} from {$this->table}  {$orderBy} {$limit}";
        else
            $sqlcmd = "select {$fields} from {$this->table} where {$whereCondition} {$orderBy} {$limit}";
    ...
    function totPages($rows = 0)
    {
        $whereCondition = $this->getWhereCondition();
        if (empty($whereCondition))
            $sqlcmd = "select count(*) as reccount from {$this->table}";
        else
            $sqlcmd = "select count(*) as reccount from {$this->table} where {$whereCondition}";
    ...
    function insert(array $data)
    {
        ...
        if (!$this->RequiredFields_Verify($data)) {
            if ($this->sqlVerbose)
                throw new Exception("ERROR on sql insert => {$this->content()}");
            return false;
        }
        ...
        try {
            ...
            if ($this->sqlVerbose)
                throw new Exception("ERROR on sql insert => {$e->getMessage()}");
    ...
    function update(int $id, array $replacements)
    {
        ...
        try {
            ...
            if ($this->sqlVerbose)
                throw new Exception("ERROR on sql update => {$e->getMessage()}");
            $this->setResult(false, $e->getMessage());
            return false;
        }
    }
    ...
    function delete(int $id)
    {
        ...
        if ($this->affected_rows() > 0)
            $this->setResult(true, "Registro Eliminado OK.");
        else
            $this->setResult(false, "NO se eliminaron registros");
    ...
    {
        if (is_readable("../app/models/datastores/$datastore.php"))
            return include "../app/models/datastores/$datastore.php";
        elseif ($datastore == "default")
            return [
            ...
            die("<h3>Datastore no encontrado $datastore");
    }
